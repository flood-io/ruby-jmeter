module RubyJmeter
  class ExtendedDSL < DSL
    ...
    def run(params = {})
      ...
      cmd = "#{params[:path] ? File.join(params[:path], 'jmeter') : 'jmeter'} #{"-n" unless params[:gui] } -t #{params[:file]} -j #{params[:log] ? params[:log] : 'jmeter.log' } -l #{params[:jtl]} -e -o #{params[:html_output]} #{build_properties(params[:properties]) if params[:properties]}"
